...
% This function draws the lens and tries to find the focal point

%You can change the shape of the lens by changing the waterline_fun and the
%lens_fun functions.

function [focal_x,focal_y] = Func_Optics(ray_num, angle_of_incidence, membrane_length, volume, stretch_coef, f1, lens_fun)
%% Variables
...
syms B
x_range = double(solve(int(sqrt(1+diff(lens_fun(x))^2),0, B)==membrane_length/2,B));
...
lens_fun_limited = symfun(lens_fun(x)*(heaviside(x+x_range)*heaviside(x-x_range)),x);
%F = finverse(lens_fun_limited)
syms y
F = solve(x==lens_fun_limited(y),y);
%disp(F)

% solve for waterlevel intercepts
disp('solving for waterlevel intercepts')
syms WL
waterlevel = solve(int(F(1),0,WL)==volume/2, WL);
...
ray_entry = linspace(water_range(1),water_range(2),ray_num+2);
